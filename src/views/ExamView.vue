<template>
  ...
  <div v-else class="exam-view">
    ...
    <!-- ══ EXIT CONFIRMATION ══ -->
    <Transition name="fade">
      <div v-if="showExitConfirm" class="modal-overlay" @click.self="stayInExam">
...
import {
  PhArrowLeft, PhArrowRight, PhFlag, PhLightbulb, PhCalculator,
...
// Called from the load-error screen (no session was ever started, so
// the route guard lets us through). Navigate back to wherever the user
// came from — practice, dashboard, etc. Fall back to practice if no history.
function smartBack() {
...
function stayInExam() {
  ...
  // Cancel the pending navigation — the user chose to stay
  if (_pendingNavNext) {
...
function exitExam() {
  ...
  showExitConfirm.value = false
  // If the exit was triggered by a router navigation (back button etc.),
  // resume it now. Otherwise push to practice ourselves.
  if (_pendingNavNext) {
    const go = _pendingNavNext
    _pendingNavNext = null
    go()           // let the original navigation proceed
  } else {
    router.replace({ name: 'practice' })
  }
}
...
onMounted(async () => {
  ...
  // Guard: intercept all navigation away from the exam at the router level.
  // This works for back button, forward, router.push — everything.
  // No history stack pollution.
  isExamActive.value = true
})

// ── Intercept navigation away while exam is active ─────────────────────
// onBeforeRouteLeave fires before the router actually navigates,
// so we can cancel it and show the confirm modal instead.
let _pendingNavNext = null   // stored callback from the guard

onBeforeRouteLeave((_to, _from, next) => {
  // If exam is done (submitted) or there is no active session, let navigation proceed
  if (!examStore.session || examStore.session?.status === 'submitted') {
    return next()
  }
  // Otherwise: save the `next` callback, show the modal, do NOT call next() yet
  _pendingNavNext = next
  showExitConfirm.value = true
  next(false)   // cancel this navigation — we'll call next() ourselves after confirmation
})
</script>
